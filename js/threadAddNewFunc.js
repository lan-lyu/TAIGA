...
/**
 * Function to create a image with all the images in the gallery formatted
 * @param {*} oneOrTwo one search or compared search
 * @param {*} gallery1Suffix gallery suffix of one search or left column in compared search
 * @param {*} gallery2Suffix null if one search
 */
function createCanvas(oneOrTwo, gallery1Suffix, gallery2Suffix) {
    var gallery1 = document.querySelector(`#search-gallery-${gallery1Suffix}`);
    var gallery2 = null;
    if (oneOrTwo == 2) {
    ...
    // we can use this to highlight the selected images
...
function generateImage(images) {
    ...
    // Set the canvas size to fit all the gallery images
    const totalWidth = Array.from(galleryImages).reduce((acc, img) => acc + img.width, 0);
    const maxHeight = Math.max(...Array.from(galleryImages).map(img => img.height));
    canvas.width = totalWidth;
    canvas.height = maxHeight;

    let currentX = 0;

    // Draw each gallery image onto the canvas
    ...
    finalImage.src = canvas.toDataURL("image/png").replace("image/png", "image/octet-stream");  // here is the most important part because if you dont replace you will get a DOM 18 exception.
    window.location.href=image; // it will save locally

    // Append the final image to the document body or a specific container
